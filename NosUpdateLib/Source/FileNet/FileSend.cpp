...
{
	std::string FileAddress = "TestData.txt";
	uint64_t startPos = 0;

	std::ifstream iFStream(FileAddress, std::ios::binary);

	uint64_t fileSize = boost::filesystem::file_size(boost::filesystem::path(FileAddress));
